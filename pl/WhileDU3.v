...
Record state: Type := {
  ...
  struct_union_info: string -> list men_var;
  type_size: type -> Z;
}.
...
{|
nrm:=(deref_sem_r (var_sem_l X)).(nrm);
err := (deref_sem_r (var_sem_l X)).(err);
|}.
...
(* 主要函数：结构体成员的正常状态 *)
Definition struct_menber_sem_nrm (x: string) (field: string) (s: state) (i: int64): Prop :=
  match s.(type_env) x with
  | TStruct x' =>
    match find_field_offset s field (s.(struct_union_info) x') with
    | Some offset =>
  ...
  Definition struct_menber_sem_err (x: string) (field: string) (s: state): Prop :=
    exists i' offset,
      s.(env) x = i' /\

      ((s.(mem) i' = None \/ s.(mem) (Int64.add i' (Int64.repr offset)) = Some (Vuninit)
      /\ Some offset =  (find_field_offset s field (s.(struct_union_info) x) ))

      \/(find_field_offset s field (s.(struct_union_info) x) ) = None).
...
{|
  nrm := struct_menber_sem_nrm x field;
  err := struct_menber_sem_err x field;
|}.

Definition union_menber_sem(  x:string)(field : string): EDenote :=
{| nrm := fun s i => True;
err := ∅;
|}.
(*
...
Fixpoint eval_r (e: expr): EDenote :=
  ...
  | EAddrOf e1 =>
      eval_l e1
  | EStructMember x field => (*x.y*)
      struct_menber_sem x field
  | EUnionMember x field =>
